defmodule ExLLM.CircuitBreaker.ConfigManager do
  @moduledoc """
  ...
  alias ExLLM.CircuitBreaker.{Bulkhead, Telemetry}
  ...
  def apply_profile(circuit_name, profile_name)
    ...
    profile_config = Map.get(@profiles, profile_name)
    GenServer.call(__MODULE__, {:apply_profile, circuit_name, profile_name, profile_config})
  end

  def apply_profile(_circuit_name, profile_name) do
  ...
  defp apply_config_to_circuit(circuit_name, config) do
    ...
    CircuitBreaker.call(circuit_name, fn -> :dummy end, cb_config)

    # Then update the configuration
    CircuitBreaker.update_config(circuit_name, cb_config)
  end

  defp apply_bulkhead_config(circuit_name, config) do
    if Map.has_key?(config, :bulkhead) do
      Bulkhead.configure(circuit_name, config.bulkhead)
    else
      :ok
    end
  end
