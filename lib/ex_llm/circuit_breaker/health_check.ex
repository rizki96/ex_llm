defmodule ExLLM.CircuitBreaker.HealthCheck do
  @moduledoc """
  ...
  ## Usage
      ...
      ExLLM.CircuitBreaker.HealthCheck.system_health()
      
      # Check specific circuit health
      ExLLM.CircuitBreaker.HealthCheck.circuit_health("api_service")
      
      # Get health summary for all circuits
      ExLLM.CircuitBreaker.HealthCheck.health_summary()
      
      # Get detailed health report
      ExLLM.CircuitBreaker.HealthCheck.health_report()
  """

  require Logger

  alias ExLLM.CircuitBreaker
  alias ExLLM.CircuitBreaker.{Adaptive, Bulkhead, Telemetry}
  ...
  defp get_telemetry_metrics(circuit_name, time_window) do
    try do
      metrics = Telemetry.get_metrics(circuit_name)
  ...
  defp get_bulkhead_metrics(circuit_name) do
    try do
      ...
      if Process.whereis(ExLLM.CircuitBreaker.Bulkhead.Registry) do
        metrics = Bulkhead.get_metrics(circuit_name)
