defmodule ExLLM.CachingInterceptor do
  @moduledoc """
  ...
  """
  def with_streaming_cache(provider, messages, options, adapter_function) when is_function(adapter_function, 0) do
    start_time = System.monotonic_time(:millisecond)
  ...
  def collect_test_responses(provider, test_cases) when is_list(test_cases) do
    ...
    results = for {messages, options} <- test_cases do
