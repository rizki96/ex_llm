defmodule ExLLM.Providers.Gemini.Embeddings do
  @moduledoc """
      ...
      request.title && request.task_type != :retrieval_document ->
        {:error,
         %{
           ...
           message: "title is only valid for RETRIEVAL_DOCUMENT task type"
         }}
  ...
  def validate_embed_request(_),
    do: {:error, %{reason: :invalid_params, message: "Invalid request"}}
  ...
  def build_embed_request_body(%EmbedContentRequest{} = request) do
    ...
    body =
      if request.task_type,
        do: Map.put(body, "taskType", task_type_to_string(request.task_type)),
        else: body
    ...
    body =
      if request.output_dimensionality,
        do: Map.put(body, "outputDimensionality", request.output_dimensionality),
        else: body
