defmodule ExLLM.Providers.Gemini.Tuning do
  @moduledoc """
  ...
  defmodule Hyperparameters do
    ...
    def to_json(%__MODULE__{} = params) do
      ...
      json =
        if params.learning_rate_multiplier do
  ...
  * `{:error, reason}` - Error details
  ...
  def stream_generate_content(model, request, opts \\ []) do
    ...
    # Create a simple callback that just passes through chunks
    callback = fn chunk -> {:ok, chunk} end
  ...
  * `{:error, reason}` - Error details
  ...
  def create_request_body(request) do
    ...
    body =
      if request[:tuning_task] do
        task = request[:tuning_task]

        tuning_task_json =
          TuningTask.to_json(%TuningTask{
    ...
    body =
      if request[:reader_project_numbers] do
