defmodule ExLLM.Core.StructuredOutputs do
  @moduledoc """
  ...
  ### Basic Example
      ...
      # Use with ExLLM
      {:ok, result} = ExLLM.Core.StructuredOutputs.chat(:anthropic, [
  ...
  ### With Retries
      ...
      {:ok, result} = ExLLM.Core.StructuredOutputs.chat(:anthropic, messages,
  ...
  ## Examples
      ...
      if ExLLM.Core.StructuredOutputs.available?() do
  ...
  ## Examples
      ...
      # With Ecto schema
      {:ok, classification} = ExLLM.Core.StructuredOutputs.chat(:anthropic, messages,
        response_model: EmailClassification,
        max_retries: 3
      )

      # With simple type spec
      {:ok, data} = ExLLM.Core.StructuredOutputs.chat(:openai, messages,
  ...
  ## Parameters
  - `response` - A Types.LLMResponse struct
  ...
  ## Examples
      ...
      {:ok, response} = ExLLM.chat(:anthropic, messages)
      {:ok, structured} = ExLLM.Core.StructuredOutputs.parse_response(response, UserProfile)
  """
  ...
  defp do_parse_response(%ExLLM.Types.LLMResponse{content: content}, response_model) do
  ...
  defp get_provider_config(provider, options) do
    config_provider =
      Keyword.get(options, :config_provider, ExLLM.Infrastructure.ConfigProvider.Env)
  ...
  ## Examples
      ...
      schema = ExLLM.Core.StructuredOutputs.simple_schema(%{
        name: :string,
        age: :integer,
        email: :string
      }, [
