defmodule ExLLM.TestResponseInterceptor do
  @moduledoc """
  ...
  alias ExLLM.TestResponseMetadata
  ...
  def intercept_request(url, body, headers, opts) do
    ...
    case TestCacheStrategy.execute(request, opts) do
      ...
      {:error, _reason} ->
        # Build basic metadata and proceed
        request_metadata = %{
          url: url,
          body: body,
          headers: headers,
  ...
  defp build_request_metadata(url, body, headers, opts) do
    %{
      url: url,
      method: Keyword.get(opts, :method, "POST"),
      headers: normalize_headers(headers),
      body_size: calculate_body_size(body),
  ...
  defp sanitize_headers(headers) do
    ...
    Enum.map(headers, fn {key, value} ->
      if key in sensitive_headers do
        {key, "[REDACTED]"}
      else
        {key, value}
      end
  ...
  defp calculate_body_size(body) when is_binary(body) do
    byte_size(body)
  end

  defp calculate_body_size(_body), do: 0
