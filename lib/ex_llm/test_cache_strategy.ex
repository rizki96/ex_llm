  ...
  defp try_fallback_selection(index, request, fallback_opts) do
    if fallback_opts.allow_older_timestamps do
      ...
      case TestCacheMatcher.find_best_match(request, cached_requests, :comprehensive) do
        {:exact, filename} -> {:fallback, filename}
        {:fuzzy, filename, similarity} when similarity > 0.8 -> {:fallback, filename}
        {:semantic, filename, similarity} when similarity > 0.7 -> {:fallback, filename}
        {:context, filename, similarity} when similarity > 0.6 -> {:fallback, filename}
        _ -> :none
      end
    else
      :none
