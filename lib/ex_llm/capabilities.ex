defmodule ExLLM.Capabilities do
  ...
  def find_providers(feature) do
    ...
    model_level = ModelCapabilities.find_models_with_features([normalized_feature])
  ...
  def display_name(capability) do
    Map.get(@display_names, capability, to_string(capability) |> String.replace("_", " ") |> String.capitalize())
  end
  ...
  def get_provider_capability_summary(provider) do
    ...
    normalized_features = provider_info.features
    |> Enum.map(&normalize_capability/1)
    |> Enum.uniq()
    
    # Get all models and their capabilities
    models = case ExLLM.list_models(provider) do
  ...
  defp get_all_features(provider_features, model_capabilities) do
    model_features = model_capabilities
    |> Map.values()
    |> List.flatten()
