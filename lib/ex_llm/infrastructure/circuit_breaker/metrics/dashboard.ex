defmodule ExLLM.Infrastructure.CircuitBreaker.Metrics.Dashboard do
  @moduledoc """
  ...
  def throughput_widget(opts \\ []) do
    ...
    throughput_data =
      ...
      |> Enum.map(fn circuit_name ->
        {:ok, data} = get_circuit_throughput(circuit_name, time_window)
  ...
  def response_time_widget(opts \\ []) do
    ...
    response_time_data =
      circuits
      |> Enum.map(fn circuit_name ->
        {:ok, data} = get_circuit_response_times(circuit_name, time_window)
        Map.put(data, :circuit_name, circuit_name)
      end)
  ...
  """
  def state_distribution_widget(_opts \\ []) do
    circuits = get_all_circuits()
  ...
  def top_failing_circuits_widget(opts \\ []) do
    ...
    failing_circuits =
      circuits
      |> Enum.map(fn circuit_name ->
        {:ok, failure_rate} = get_circuit_failure_rate(circuit_name, time_window)
  ...
  defp build_circuit_grid(circuit_summaries) do
    ...
    |> Enum.sort_by(&{&1.needs_attention, &1.health_score}, fn
      {true, _score1}, {false, _} -> true
      {false, _}, {true, _score2} -> false
