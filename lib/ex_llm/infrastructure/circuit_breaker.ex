defmodule ExLLM.Infrastructure.CircuitBreaker do
  @moduledoc """
  ...
  def apply_profile(circuit_name, profile_name) do
    ExLLM.Infrastructure.CircuitBreaker.ConfigManager.apply_profile(circuit_name, profile_name)
  end
  ...
  def update_circuit_config(circuit_name, config_changes) do
    ExLLM.Infrastructure.CircuitBreaker.ConfigManager.update_circuit(circuit_name, config_changes)
  end
  ...
  def get_circuit_config(circuit_name) do
    ExLLM.Infrastructure.CircuitBreaker.ConfigManager.get_config(circuit_name)
  end
  ...
  def rollback_config(circuit_name) do
    ExLLM.Infrastructure.CircuitBreaker.ConfigManager.rollback(circuit_name)
  end
  ...
  def call_with_bulkhead(circuit_name, fun, opts \\ []) when is_function(fun, 0) do
    ExLLM.Infrastructure.CircuitBreaker.Bulkhead.execute(circuit_name, fun, opts)
  end
  ...
  defp build_config(opts) when is_map(opts) do
    %{
      failure_threshold: Map.get(opts, :failure_threshold, 5),
      success_threshold: Map.get(opts, :success_threshold, 3),
      reset_timeout: Map.get(opts, :reset_timeout, 30_000),
