defmodule ExLLM.Cache.Storage.TestCache do
  @moduledoc """
  ...
  def clear(:all) do
    ...
    case File.rm_rf(cache_base_dir) do
      {:ok, _} -> :ok
      {:error, reason} -> {:error, "Failed to clear all cache: #{inspect(reason)}"}
    end
  end
  ...
  def clear(cache_key) do
    ...
    case File.rm_rf(cache_dir) do
      {:ok, _} -> :ok
      {:error, reason} -> {:error, "Failed to clear cache for #{cache_key}: #{inspect(reason)}"}
    end
  end
