defmodule ExLLM.Adapters.Shared.StreamingCoordinator do
  ...
  def create_stream_collector(callback, parse_chunk_fn, recovery_id, options) do
    ...
    fn
      {:data, data}, acc ->
        ...
        {buffer, chunk_count} = case acc do
          {_, _} = state -> state
          _ -> {"", 0}
        end
        
        {new_buffer, new_count} = process_stream_data(
          data, 
          buffer, 
          callback, 
          parse_chunk_fn, 
  ...
  """
  def process_stream_data(data, buffer, callback, parse_chunk_fn, recovery_id, chunk_count, options) do
    ...
    chunks_received = Enum.reduce(complete_lines, chunk_count, fn line, count ->
