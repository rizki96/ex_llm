defmodule ExLLM.Adapters.Shared.ModelFetcher do
  @moduledoc """
  ...
  """
  @callback transform_model_config(model_id :: atom() | String.t(), config :: map()) :: Types.Model.t()
  ...
  """
  @spec list_models_with_loader(module(), atom(), keyword()) :: {:ok, list(Types.Model.t())} | {:error, term()}
  def list_models_with_loader(adapter_module, provider, options \\ []) do
    ...
    ExLLM.ModelLoader.load_models(provider,
  ...
  """
  @spec fetch_openai_compatible_models(String.t(), String.t(), keyword()) :: {:ok, list(map())} | {:error, term()}
  ...
  def standard_model_filter(model, options \\ []) do
    ...
    passes_include = if Enum.empty?(includes) do
      true
    else
  ...
  def transform_standard_config(model_id, config, provider) do
    %Types.Model{
      ...
      description: Map.get(config, :description, ModelUtils.generate_description(to_string(model_id), provider)),
  ...
  def process_models(raw_models, adapter_module, provider, options \\ []) do
    filter_fn = if function_exported?(adapter_module, :filter_model, 1) do
      &adapter_module.filter_model/1
    else
      &standard_model_filter(&1, options)
    end
    
    parse_fn = if function_exported?(adapter_module, :parse_api_model, 1) do
  ...
  defp get_transformer(adapter_module, model_id, config) do
    if function_exported?(adapter_module, :transform_model_config, 2) do
      ...
      provider = adapter_module.__provider__()  # Assumes adapter stores this
  ...
  defp parse_model_capabilities(model, provider, _options) do
    ...
    # Build features list
    features = []
    features = if base_capabilities.supports_streaming, do: ["streaming" | features], else: features
    features = if base_capabilities.supports_functions, do: ["function_calling" | features], else: features
