if Code.ensure_loaded?(OpenTelemetry) and Code.ensure_loaded?(OpenTelemetry.Tracer) do
  defmodule ExLLM.Telemetry.OpenTelemetry do
    @moduledoc """
    ...
    ## Example
        ...
        Task.async(fn ->
          ExLLM.Telemetry.OpenTelemetry.with_context(ctx, fn ->
  ...
  defmodule ExLLM.Telemetry.OpenTelemetry do
    ...
    def stream_chat(model_or_config, messages, opts \\ []) do
      ExLLM.stream_chat(model_or_config, messages, opts)
    end

    def embed(model_or_config, input, opts \\ []) do
      ExLLM.embed(model_or_config, input, opts)
    end
