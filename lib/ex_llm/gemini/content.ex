defmodule ExLLM.Gemini.Content do
  @moduledoc """
  ...
  defmodule GenerateContentRequest do
    ...
    def to_json(%__MODULE__{} = request) do
      json = %{
        "contents" => Enum.map(request.contents, &ExLLM.Gemini.Content.content_to_json/1)
      }
      
      json = if request.model, do: Map.put(json, "model", request.model), else: json
      json = if request.system_instruction, do: Map.put(json, "systemInstruction", ExLLM.Gemini.Content.content_to_json(request.system_instruction)), else: json
      json = if request.generation_config, do: Map.put(json, "generationConfig", generation_config_to_json(request.generation_config)), else: json
      json = if request.safety_settings, do: Map.put(json, "safetySettings", Enum.map(request.safety_settings, &safety_setting_to_json/1)), else: json
      json = if request.tools, do: Map.put(json, "tools", Enum.map(request.tools, &tool_to_json/1)), else: json
      json = if request.tool_config, do: Map.put(json, "toolConfig", tool_config_to_json(request.tool_config)), else: json
    ...
    defp generation_config_to_json(config) when is_struct(config, GenerationConfig) do
      json = %{}
      json = if config.temperature, do: Map.put(json, "temperature", config.temperature), else: json
      ...
      json = if config.candidate_count, do: Map.put(json, "candidateCount", config.candidate_count), else: json
      json = if config.max_output_tokens, do: Map.put(json, "maxOutputTokens", config.max_output_tokens), else: json
      json = if config.stop_sequences, do: Map.put(json, "stopSequences", config.stop_sequences), else: json
      json = if config.response_mime_type, do: Map.put(json, "responseMimeType", config.response_mime_type), else: json
      json = if config.response_schema, do: Map.put(json, "responseSchema", config.response_schema), else: json
    ...
    defp generation_config_to_json(config) when is_map(config) do
      ...
      json = %{}
      json = if Map.has_key?(config, :temperature), do: Map.put(json, "temperature", config.temperature), else: json
      ...
      json = if Map.has_key?(config, :candidate_count), do: Map.put(json, "candidateCount", config.candidate_count), else: json
      json = if Map.has_key?(config, :max_output_tokens), do: Map.put(json, "maxOutputTokens", config.max_output_tokens), else: json
      json = if Map.has_key?(config, :stop_sequences), do: Map.put(json, "stopSequences", config.stop_sequences), else: json
      json = if Map.has_key?(config, :response_mime_type), do: Map.put(json, "responseMimeType", config.response_mime_type), else: json
      json = if Map.has_key?(config, :response_schema), do: Map.put(json, "responseSchema", config.response_schema), else: json
    ...
    with :ok <- validate_request(request),
      ...
      Task.start(fn ->
        case Req.post(url,
          ...
          {:error, reason} ->
            send(parent, {:stream_error, {:error, %{reason: :network_error, message: inspect(reason)}}})
        end
      end)
  ...
  defp validate_request(%GenerateContentRequest{contents: contents}) do
    ...
    invalid_role = Enum.find(contents, fn content ->
  ...
  defp normalize_model_name(nil), do: {:error, %{reason: :invalid_params, message: "Model name is required"}}
  defp normalize_model_name(""), do: {:error, %{reason: :invalid_params, message: "Model name is required"}}
  ...
  defp build_request_body(request) do
    ...
    body = if request.system_instruction do
      Map.put(body, "systemInstruction", serialize_content(request.system_instruction))
    else
      body
    end
    
    body = if request.generation_config do
      Map.put(body, "generationConfig", serialize_generation_config(request.generation_config))
    else
      body
    end
    
    body = if request.safety_settings do
      Map.put(body, "safetySettings", Enum.map(request.safety_settings, &serialize_safety_setting/1))
    else
      body
    end
    
    body = if request.tools do
      Map.put(body, "tools", Enum.map(request.tools, &serialize_tool/1))
    else
      body
    end
    
    body = if request.tool_config do
  ...
  defp serialize_generation_config(config) do
    ...
    config_map = if config.temperature, do: Map.put(config_map, "temperature", config.temperature), else: config_map
    ...
    config_map = if config.candidate_count, do: Map.put(config_map, "candidateCount", config.candidate_count), else: config_map
    config_map = if config.max_output_tokens, do: Map.put(config_map, "maxOutputTokens", config.max_output_tokens), else: config_map
    config_map = if config.stop_sequences, do: Map.put(config_map, "stopSequences", config.stop_sequences), else: config_map
    config_map = if config.response_mime_type, do: Map.put(config_map, "responseMimeType", config.response_mime_type), else: config_map
    config_map = if config.response_schema, do: Map.put(config_map, "responseSchema", config.response_schema), else: config_map
    config_map = if config.thinking_config, do: Map.put(config_map, "thinkingConfig", config.thinking_config), else: config_map
  ...
  defp serialize_tool(tool) do
    ...
    tool_map = if tool.function_declarations, do: Map.put(tool_map, "functionDeclarations", tool.function_declarations), else: tool_map
    tool_map = if tool.google_search, do: Map.put(tool_map, "googleSearch", tool.google_search), else: tool_map
    tool_map = if tool.code_execution, do: Map.put(tool_map, "codeExecution", tool.code_execution), else: tool_map
  ...
  defp handle_stream_response(response, parent, buffer) do
    ...
    receive do
      ...
      {^ref, {:error, reason}} ->
        send(parent, {:stream_error, {:error, %{reason: :stream_error, message: inspect(reason)}}})
    after
      30_000 ->
