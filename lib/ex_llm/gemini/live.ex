defmodule ExLLM.Gemini.Live do
  @moduledoc """
  ...
  defp parse_tool_call(tool_call) do
    function_calls = Enum.map(tool_call["functionCalls"] || [], fn call ->
      %FunctionCall{
        id: call["id"],
        name: call["name"],
        args: call["args"]
      ...
      {:ok, conn_pid} ->
        case :gun.await_up(conn_pid) do
          {:ok, :http} ->
            ...
            stream_ref = :gun.ws_upgrade(conn_pid, uri.path <> (uri.query && "?#{uri.query}" || ""), headers)
