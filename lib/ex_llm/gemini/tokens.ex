defmodule ExLLM.Gemini.Tokens do
  ...
  def to_json(%CountTokensRequest{} = request) do
    ...
    json = 
      if request.contents do
        Map.put(json, "contents", Enum.map(request.contents, &ExLLM.Gemini.Content.content_to_json/1))
      else
        json
      end
    
    if request.generate_content_request do
      Map.put(json, "generateContentRequest", GenerateContentRequest.to_json(request.generate_content_request))
    else
      json
    end
  end
  ...
  def normalize_model_name(nil), do: {:error, %{reason: :invalid_params, message: "Model name is required"}}
  ...
  def normalize_model_name(_), do: {:error, %{reason: :invalid_params, message: "Model name must be a string"}}
