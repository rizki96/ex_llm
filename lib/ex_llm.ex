defmodule ExLLM do
  ...
  def load_session(path_or_json) when is_binary(path_or_json) do
    ...
    if String.starts_with?(String.trim(path_or_json), "{") and String.ends_with?(String.trim(path_or_json), "}") do
  ...
  def chat_session(session, content, opts \\ []) do
    ...
    provider = if is_binary(session.llm_backend), do: String.to_atom(session.llm_backend), else: session.llm_backend
