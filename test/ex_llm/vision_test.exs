defmodule ExLLM.VisionTest do
  ...
  describe "build_message/4" do
    test "creates message with single image URL" do
      {:ok, message} = Vision.build_message("user", "What's in this image?", ["https://example.com/image.jpg"])
    ...
    test "creates message with image details" do
      {:ok, message} = Vision.build_message("user", "Look", ["https://example.com/img.jpg"], detail: "high")
  ...
  describe "format_for_provider/2" do
    test "formats for OpenAI provider" do
      ...
      assert formatted == messages  # OpenAI format is the default
    end
  ...
  describe "load_image/2" do
    test "loads image from file" do
      ...
      # JPEG magic bytes
      File.write!(path, <<0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46, 0x49, 0x46, 0x00, 0x01>>)
  ...
  describe "normalize_messages/1" do
    test "normalizes atom types to strings" do
      ...
      assert text_part.type == "text"  # Was :text
      assert image_part.type == "image_url"  # Was :image_url
    end
  end
