defmodule ExLLM.Gemini.ContentTest do
  ...
  describe "generate_content/3" do
    ...
    test "generates content with system instruction" do
      ...
      assert {:ok, response} = Content.generate_content(model, request)
      assert response.candidates |> hd() |> Map.get(:content) |> Map.get(:parts) |> hd() |> Map.get(:text) =~ ~r/4|four/i
    end
    ...
    test "handles multi-turn conversation" do
      ...
      assert {:ok, response} = Content.generate_content(model, request)
      assert response.candidates |> hd() |> Map.get(:content) |> Map.get(:parts) |> hd() |> Map.get(:text) =~ "Alice"
    end
    ...
    test "generates JSON response with response_mime_type" do
      ...
      assert {:ok, response} = Content.generate_content(model, request)
      json_text = response.candidates |> hd() |> Map.get(:content) |> Map.get(:parts) |> hd() |> Map.get(:text)
    ...
    test "validates request structure" do
      ...
      # Empty contents
      assert {:error, error} = Content.generate_content(model, %GenerateContentRequest{contents: []})
  ...
  describe "stream_generate_content/3" do
    ...
    test "handles streaming errors" do
      ...
      request = %GenerateContentRequest{
        ...
        generation_config: %GenerationConfig{
          max_output_tokens: -1  # Invalid value
        }
      }
  ...
  describe "multimodal content" do
    test "generates content with image input" do
      ...
      assert {:ok, response} = Content.generate_content(model, request)
      response_text = response.candidates |> hd() |> Map.get(:content) |> Map.get(:parts) |> hd() |> Map.get(:text)
