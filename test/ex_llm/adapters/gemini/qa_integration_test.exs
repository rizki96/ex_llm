defmodule ExLLM.Gemini.QAIntegrationTest do
  ...
  describe "generate_answer/3 with API" do
    ...
    test "generates answer with inline passages", %{api_key: api_key} do
      ...
      passages = [
        %{
          id: "france_info",
          content: %{
            parts: [%{text: "France is a country in Western Europe. Paris is the capital city of France and also its largest city. The city is known for landmarks like the Eiffel Tower and the Louvre Museum."}]
          }
        }
      ]
      ...
      {:ok, response} = QA.generate_answer("models/gemini-1.5-flash", contents, :abstractive, opts)
    ...
    test "handles question with insufficient grounding", %{api_key: api_key} do
      ...
      passages = [
        %{
          id: "earth_info",
          content: %{
            parts: [%{text: "Earth is the third planet from the Sun and has a population of approximately 8 billion people as of 2024."}]
          }
        }
      ]
    ...
    test "handles different answer styles", %{api_key: api_key} do
      ...
      passages = [
        %{
          id: "ml_definition",
          content: %{
            parts: [%{text: "Machine learning is a subset of artificial intelligence that enables computers to learn and make decisions from data without being explicitly programmed. It involves algorithms that can identify patterns in data and make predictions or classifications based on those patterns."}]
          }
        }
      ]
    ...
    test "handles multi-turn conversation", %{api_key: api_key} do
      contents = [
        ...
        %{
          parts: [%{text: "Renewable energy comes from natural sources that replenish themselves, like solar and wind power."}],
          role: "model"
        },
        %{
      ...
      passages = [
        %{
          id: "solar_benefits",
          content: %{
            parts: [%{text: "Solar energy benefits include: 1) It's environmentally clean with no emissions, 2) It reduces electricity bills over time, 3) It's renewable and inexhaustible, 4) It requires minimal maintenance once installed, 5) It can increase property values."}]
          }
        }
      ]
      ...
      assert String.contains?(downcase_answer, "solar") || String.contains?(downcase_answer, "benefit")
    end
    ...
    test "handles safety filtering", %{api_key: api_key} do
      ...
      passages = [
        %{
          id: "peace_info",
          content: %{
            parts: [%{text: "Peaceful resolutions involve dialogue, negotiation, and compromise to solve conflicts without violence."}]
          }
        }
      ]
    ...
    test "handles API errors gracefully", %{api_key: _api_key} do
      ...
      {:error, error} = QA.generate_answer("models/gemini-1.5-flash", contents, :abstractive, opts)
    ...
    test "handles invalid model name", %{api_key: api_key} do
      ...
      {:error, error} = QA.generate_answer("models/nonexistent-model", contents, :abstractive, opts)
  ...
  describe "generate_answer/3 with semantic retriever" do
    ...
    test "generates answer using semantic retriever", %{oauth_token: oauth_token, test_corpus: test_corpus} do
      contents = [
        %{
