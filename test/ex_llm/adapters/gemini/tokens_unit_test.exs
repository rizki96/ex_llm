defmodule ExLLM.Gemini.TokensUnitTest do
  ...
  describe "to_json/1 for CountTokensRequest" do
    test "serializes request with contents" do
      ...
      assert json["contents"] |> hd() |> Map.get("parts") |> hd() |> Map.get("text") == "Hello world"
  ...
  describe "normalize_model_name/1" do
    test "handles various model name formats" do
      ...
      assert Tokens.normalize_model_name("models/gemini-2.0-flash") == {:ok, "models/gemini-2.0-flash"}
      assert Tokens.normalize_model_name("gemini/gemini-2.0-flash") == {:ok, "models/gemini-2.0-flash"}
