defmodule ExLLM.Adapters.Gemini.IntegrationTest do
  ...
  alias ExLLM.Gemini.{Models, Content, Files, Caching, Embeddings, Tuning, Permissions, Corpus, Document, Chunk}
  ...
  describe "Gemini adapter integration with ExLLM interfaces" do
    ...
    test "basic chat functionality through ExLLM interface" do
      ...
      {:ok, response} = ExLLM.chat(messages, 
        provider: :gemini,
    ...
    test "streaming chat through ExLLM interface" do
      ...
      {:ok, stream} = ExLLM.stream_chat(messages,
        provider: :gemini,
        config_provider: ExLLM.ConfigProvider.Env,
        model: "gemini-2.0-flash"
      )
    ...
    test "error handling integration" do
      ...
      {:error, error} = ExLLM.chat(messages, 
        provider: :gemini,
  ...
  describe "Cross-feature interactions" do
    test "API modules validation and structure" do
      ...
      request = %ExLLM.Gemini.Content.GenerateContentRequest{
        contents: [%ExLLM.Gemini.Content.Content{role: "user", parts: [%ExLLM.Gemini.Content.Part{text: "Hello"}]}]
      }
      assert {:error, _} = Content.generate_content("models/gemini-pro", request, api_key: "invalid")
      ...
      assert {:error, _} = Embeddings.embed_content("text-embedding-004", request, api_key: "invalid")
    ...
    test "embeddings integration with similarity search" do
      ...
      {:ok, embedding_1} = ExLLM.embeddings(:gemini, [content_1],
        config_provider: ExLLM.ConfigProvider.Env,
        model: "text-embedding-004"
      )
      
      {:ok, embedding_2} = ExLLM.embeddings(:gemini, [content_2],
        config_provider: ExLLM.ConfigProvider.Env,
        model: "text-embedding-004"
      )
      
      {:ok, embedding_3} = ExLLM.embeddings(:gemini, [content_3],
        config_provider: ExLLM.ConfigProvider.Env,
        model: "text-embedding-004"
      )
  ...
  describe "Error handling and retries" do
    test "handles API errors gracefully" do
      ...
      {:error, error} = ExLLM.chat(:gemini, messages,
  ...
  describe "Performance characteristics" do
    ...
    test "response time benchmarks" do
      ...
      {:ok, _response} = ExLLM.chat(messages,
        provider: :gemini,
        config_provider: ExLLM.ConfigProvider.Env,
        model: "gemini-2.0-flash"
      )
