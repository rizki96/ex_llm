defmodule ExLLM.Adapters.Gemini.ChunkIntegrationTest do
  ...
  describe "Chunk API integration" do
    ...
    test "batch operations work correctly" do
      ...
      assert {:ok, batch_result} = Chunk.batch_create_chunks(parent, chunk_requests, api_key: @api_key)
      ...
      assert {:ok, update_result} = Chunk.batch_update_chunks(parent, update_requests, api_key: @api_key)
    ...
    test "returns error for invalid document name in create" do
      ...
      assert {:error, %{message: message}} = Chunk.create_chunk(invalid_parent, params, api_key: @api_key)
    ...
    test "returns error for invalid chunk name in update" do
      ...
      assert {:error, %{message: message}} = Chunk.update_chunk(invalid_name, updates, opts, api_key: @api_key)
