defmodule ExLLM.Adapters.Gemini.ChunkTest do
  ...
  describe "build_create_request/1" do
    ...
    test "builds create request with custom metadata" do
      ...
      assert %{key: "keywords", stringListValue: %{values: ["AI", "ML", "research"]}} = keywords_meta
    end
  end
  ...
  describe "validation" do
    test "validates document name format" do
      ...
      assert {:error, %{message: _}} = Chunk.validate_document_name("corpora/test-corpus/documents/")
      assert {:error, %{message: _}} = Chunk.validate_document_name("corpora/test-corpus/documents/-invalid")
    end

    test "validates chunk name format" do
      # Valid chunk names
      assert :ok == Chunk.validate_chunk_name("corpora/test-corpus/documents/test-doc/chunks/chunk-123")
      assert :ok == Chunk.validate_chunk_name("corpora/corpus-456/documents/doc-789/chunks/abc123def")
      ...
      assert {:error, %{message: _}} = Chunk.validate_chunk_name("corpora/test/documents/doc/chunks/")
      assert {:error, %{message: _}} = Chunk.validate_chunk_name("corpora/test/documents/doc/chunks/-invalid")
      assert {:error, %{message: _}} = Chunk.validate_chunk_name("corpora/test/documents/doc/chunks/invalid-")
    end
    ...
    test "validates create params" do
      ...
      # Empty string value
      assert {:error, %{message: message}} = Chunk.validate_create_params(%{data: %{string_value: ""}})
    ...
    test "validates update params" do
      ...
      assert :ok == Chunk.validate_update_params(%{data: %{string_value: "New content"}}, %{update_mask: "data"})
      assert :ok == Chunk.validate_update_params(%{custom_metadata: []}, %{update_mask: "customMetadata"})
      
      # Missing update mask
      assert {:error, %{message: message}} = Chunk.validate_update_params(%{data: %{string_value: "New"}}, %{})
      assert String.contains?(message, "updateMask is required")
      
      # Invalid update mask
      assert {:error, %{message: message}} = Chunk.validate_update_params(%{data: %{string_value: "New"}}, %{update_mask: "invalidField"})
      assert String.contains?(message, "only supports updating data and customMetadata")
      
      # Empty string value when updating data
      assert {:error, %{message: message}} = Chunk.validate_update_params(%{data: %{string_value: ""}}, %{update_mask: "data"})
      assert String.contains?(message, "string_value cannot be empty")
    end
