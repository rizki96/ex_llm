defmodule ExLLM.Adapters.MockTest do
  ...
  describe "chat/2" do
    ...
    test "cycles through responses when list is provided" do
      ...
      assert {:ok, %{content: "Response 1"}} = Mock.chat(messages, [])  # Cycles back
    end
  end
  ...
  describe "embeddings/2" do
    ...
    test "returns default embeddings when not configured" do
      ...
      assert length(hd(response.embeddings)) == 384  # Default mock embedding size
      assert response.model == "mock-embedding-model"  # Default mock model
    end
  end
  ...
  describe "list_embedding_models/1" do
    ...
    test "returns default embedding models when not configured" do
      ...
      assert is_list(models)
      assert length(models) > 0
      assert hd(models).name == "mock-embedding-small"  # Default mock embedding model
    end
  end
  ...
  describe "complex scenarios" do
    ...
    test "simulates vision model response" do
      vision_response = fn messages, _options ->
        ...
        has_image = Enum.any?(messages, fn msg ->
          case msg.content do
