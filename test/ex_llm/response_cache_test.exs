defmodule ExLLM.ResponseCacheTest do
  ...
  describe "response caching" do
    test "stores and retrieves responses" do
      ...
      assert get_in(cached_entry.response_data, [:choices, Access.at(0), :message, :content]) == "Hello! How can I help you?"
    ...
    test "caching interceptor wraps function calls" do
      ...
      request_data = %{}  # Empty since we can't capture the actual request in this simple test
    ...
    test "handles streaming cache correctly" do
      ...
      chunks = [
        ...
        %ExLLM.Types.StreamChunk{content: "!", finish_reason: "stop", model: "test-model", id: "test-123"}
      ]
      ...
      {:ok, stream} = CachingInterceptor.with_streaming_cache(:test_provider, messages, options, test_stream_function)
      ...
      cached = ResponseCache.get_response("test_provider", "streaming", %{
        messages: messages,
        model: "test-model"
      })
      
      assert cached != nil
      assert cached.response_data[:streaming_chunks] != nil or cached.response_data["streaming_chunks"] != nil
  ...
  describe "test scenario collection" do
    ...
    test "create_test_collection organizes responses by category" do
      ...
      Mock.set_response_handler(fn messages, _options ->
        user_message = List.last(messages)
        content = if is_binary(user_message.content), do: user_message.content, else: "complex content"
