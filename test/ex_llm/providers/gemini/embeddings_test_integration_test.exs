defmodule ExLLM.Gemini.EmbeddingsTest do
  ...
  describe "embed_content/3" do
    ...
    test "generates embedding with specific task type" do
      request = %EmbedContentRequest{
        content: %Content{role: "user", parts: [%Part{text: "What is the capital of France?"}]},
    ...
    test "handles different task types" do
      ...
      for task_type <- task_types do
        request = %EmbedContentRequest{
          content: %Content{role: "user", parts: [%Part{text: "Testing #{task_type} task type"}]},
          task_type: task_type
        }
  ...
  describe "batch_embed_contents/2" do
    test "successfully generates embeddings for multiple texts" do
      requests = [
        %EmbedContentRequest{
          ...
          content: %Content{role: "user", parts: [%Part{text: "First text to embed"}]}
        },
        %EmbedContentRequest{
          model: "models/text-embedding-004",
          content: %Content{role: "user", parts: [%Part{text: "Second text to embed"}]}
        },
        %EmbedContentRequest{
          model: "models/text-embedding-004",
          content: %Content{role: "user", parts: [%Part{text: "Third text to embed"}]}
        }
      ]
    ...
    test "generates batch embeddings with different task types" do
      requests = [
        %EmbedContentRequest{
          model: "models/text-embedding-004",
          content: %Content{role: "user", parts: [%Part{text: "What is machine learning?"}]},
          task_type: :retrieval_query
        },
        %EmbedContentRequest{
          model: "models/text-embedding-004",
  ...
  describe "model compatibility" do
    test "older model (embedding-001) does not support task type" do
      request = %EmbedContentRequest{
        content: %Content{role: "user", parts: [%Part{text: "Test for older model"}]},
        task_type: :retrieval_query
      }
