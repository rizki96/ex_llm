defmodule ExLLM.Core.CostTest do
  use ExUnit.Case

  describe "cost calculation" do
    test "calculates cost for OpenAI GPT-4o" do
      usage = %{input_tokens: 1000, output_tokens: 500}
      result = ExLLM.Core.Cost.calculate(:openai, "gpt-4o", usage)
    ...
    test "calculates cost for Anthropic Claude" do
      ...
      result = ExLLM.Core.Cost.calculate(:anthropic, "claude-3-5-sonnet-20241022", usage)
    ...
    test "handles unknown provider/model" do
      usage = %{input_tokens: 1000, output_tokens: 500}
      result = ExLLM.Core.Cost.calculate(:unknown, "model", usage)
  ...
  describe "token estimation" do
    test "estimates tokens for simple text" do
      tokens = ExLLM.Core.Cost.estimate_tokens("Hello world")
      assert tokens == 3
    end
    ...
    test "estimates tokens for empty text" do
      tokens = ExLLM.Core.Cost.estimate_tokens("")
      assert tokens == 0
    end

    test "estimates tokens for message map" do
      message = %{content: "Hello world"}
      tokens = ExLLM.Core.Cost.estimate_tokens(message)
      assert tokens == 3
    end
    ...
    test "estimates tokens for message list" do
      ...
      tokens = ExLLM.Core.Cost.estimate_tokens(messages)
      assert tokens == 10
    end
  end
  ...
  describe "cost formatting" do
    test "formats small costs correctly" do
      assert ExLLM.Core.Cost.format(0.005) == "$0.005000"
      assert ExLLM.Core.Cost.format(0.0035) == "$0.003500"
      assert ExLLM.Core.Cost.format(0.000324) == "$0.000324"
    end

    test "formats medium costs correctly" do
      assert ExLLM.Core.Cost.format(0.0235) == "$0.0235"
      assert ExLLM.Core.Cost.format(0.5) == "$0.5000"
    end

    test "formats large costs correctly" do
      assert ExLLM.Core.Cost.format(1.50) == "$1.50"
      assert ExLLM.Core.Cost.format(25.99) == "$25.99"
    end
  end
end
