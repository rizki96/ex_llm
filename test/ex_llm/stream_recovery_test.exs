defmodule ExLLM.StreamRecoveryTest do
  ...
  describe "recovery ID generation" do
    test "generates unique recovery IDs" do
      ...
      assert id1 != id2  # Should be unique even with same inputs
    end
  end
  ...
  describe "saving partial responses" do
    test "saves stream chunks" do
      ...
      {:ok, recovery_id} = StreamRecovery.init_recovery(:mock, [%{role: "user", content: "test"}], [])
...
# 
  ...
  describe "concurrent access" do
    ...
    test "handles concurrent operations on different recovery IDs" do
      tasks = for i <- 1..50 do
        Task.async(fn ->
          {:ok, recovery_id} = StreamRecovery.init_recovery(:mock, [%{role: "user", content: "test #{i}"}], [])
  ...
  describe "memory management" do
    test "old recovery data is eventually cleaned up" do
      ...
      test_id = Enum.at(recovery_ids, 499)  # Get the 500th ID
