defmodule ExLLM.ResponseCaptureTest do
  ...
  describe "capture_response/5" do
    test "captures response when enabled" do
      ...
      assert :ok = ResponseCapture.capture_response(provider, endpoint, request, response, metadata)
      ...
      capture_key = Enum.find(keys, fn key ->
    ...
    test "skips capture when disabled" do
      ...
      initial_keys = LiveApiCacheStorage.list_cache_keys()
      |> Enum.filter(fn key ->
      ...
      assert :ok = ResponseCapture.capture_response(:test_provider, "/test/endpoint", %{}, %{}, %{})
      
      # Wait a bit
      Process.sleep(100)
      
      # Verify nothing new was stored
      final_keys = LiveApiCacheStorage.list_cache_keys()
      |> Enum.filter(fn key ->
        String.contains?(key, "test_provider") && String.contains?(key, "/test/endpoint")
      end)
      |> length()
    ...
    test "displays capture when display is enabled" do
      ...
      assert captured_output =~ "openai"  # Provider name appears in output
      assert captured_output =~ "123ms"    # Duration appears
