defmodule ExLLM.CircuitBreaker.AdaptiveTest do
  use ExUnit.Case
  ...
  setup do
    ...
    if Process.whereis(ExLLM.CircuitBreaker.Adaptive) do
      GenServer.stop(ExLLM.CircuitBreaker.Adaptive)
    end
  ...
  describe "adaptive circuit breaker" do
    test "starts when enabled in config" do
      {:ok, pid} =
        ExLLM.CircuitBreaker.Adaptive.start_link(
          enabled: true,
          min_threshold: 2,
          max_threshold: 10,
    ...
    test "adjusts thresholds based on error rate" do
      ...
      ExLLM.CircuitBreaker.call("adaptive_test", fn -> :ok end,
      ...
      for _ <- 1..8 do
        ExLLM.CircuitBreaker.call("adaptive_test", fn -> raise "error" end)
      end
      ...
      ExLLM.CircuitBreaker.Adaptive.update_thresholds()

      # Check that threshold was lowered (66% error rate should trigger decrease)
      {:ok, stats} = ExLLM.CircuitBreaker.get_stats("adaptive_test")
    ...
    test "raises threshold for low error rate" do
      ...
      ExLLM.CircuitBreaker.call("low_error", fn -> :ok end, failure_threshold: 5)

      # 99% success rate (very low error rate triggers increase)
      for _ <- 1..99 do
        ExLLM.CircuitBreaker.call("low_error", fn -> :ok end)
      end

      for _ <- 1..1 do
        ExLLM.CircuitBreaker.call("low_error", fn -> raise "error" end)
      end
      ...
      {:ok, stats} = ExLLM.CircuitBreaker.get_stats("low_error")
    ...
    test "respects min and max thresholds" do
      ...
      # Test min threshold
      ExLLM.CircuitBreaker.call("min_test", fn -> :ok end, failure_threshold: 4)

      # Force 100% error rate
      for _ <- 1..10 do
        ExLLM.CircuitBreaker.call("min_test", fn -> raise "error" end)
      end

      ExLLM.CircuitBreaker.Adaptive.update_thresholds()

      {:ok, stats} = ExLLM.CircuitBreaker.get_stats("min_test")
      assert stats.config.failure_threshold >= 3

      # Test max threshold
      ExLLM.CircuitBreaker.reset("min_test")
      ExLLM.CircuitBreaker.update_config("min_test", failure_threshold: 6)

      # Force 0% error rate
      for _ <- 1..20 do
        ExLLM.CircuitBreaker.call("min_test", fn -> :ok end)
      end
    ...
    test "emits telemetry event on threshold update" do
      ...
      ExLLM.CircuitBreaker.call("telemetry_test", fn -> :ok end,
        # High threshold so circuit doesn't open
        failure_threshold: 20
      )
      ...
      for _ <- 1..10 do
        ExLLM.CircuitBreaker.call("telemetry_test", fn -> raise "error" end)
      end
  ...
  describe "metrics calculation" do
    test "calculates error rate correctly" do
      ...
      for _ <- 1..7 do
        ExLLM.CircuitBreaker.call("calc_test", fn -> :ok end)
      end

      for _ <- 1..3 do
        ExLLM.CircuitBreaker.call("calc_test", fn -> raise "error" end)
      end

      # Need to manually store metrics since adaptive isn't automatically triggered
      telemetry_metrics = ExLLM.CircuitBreaker.Telemetry.get_metrics("calc_test")

      metrics = ExLLM.CircuitBreaker.Adaptive.get_circuit_metrics("calc_test")
    ...
    test "handles zero calls gracefully" do
      metrics = ExLLM.CircuitBreaker.Adaptive.get_circuit_metrics("empty_circuit")
