defmodule ExLLM.Infrastructure.CircuitBreaker.BulkheadTest do
  use ExUnit.Case
  ...
  describe "bulkhead initialization" do
    test "creates Registry for bulkhead workers" do
      assert Process.whereis(ExLLM.Infrastructure.CircuitBreaker.Bulkhead.Registry) != nil
    end

    test "configures default concurrency limits" do
      config = ExLLM.Infrastructure.CircuitBreaker.Bulkhead.get_config("test_circuit")
  ...
  describe "concurrency limiting" do
    test "enforces max concurrent requests" do
      ...
      ExLLM.Infrastructure.CircuitBreaker.Bulkhead.configure("limited",
        max_concurrent: 2,
        max_queued: 0
      )
      ...
      task1 =
        Task.async(fn ->
          ExLLM.Infrastructure.CircuitBreaker.call_with_bulkhead("limited", fn ->
    ...
    test "queues requests when bulkhead is full" do
      ExLLM.Infrastructure.CircuitBreaker.Bulkhead.configure("queued",
        max_concurrent: 1,
        max_queued: 2,
        queue_timeout: 1000
      )
      ...
      task1 =
        Task.async(fn ->
          ExLLM.Infrastructure.CircuitBreaker.call_with_bulkhead("queued", fn ->
            send(parent, {:started, 1})
      ...
      metrics = ExLLM.Infrastructure.CircuitBreaker.Bulkhead.get_metrics("queued")
    ...
    test "rejects requests when queue is full" do
      ExLLM.Infrastructure.CircuitBreaker.Bulkhead.configure("full_queue",
        max_concurrent: 1,
        max_queued: 1,
        queue_timeout: 1000
      )
      ...
      _task1 =
        Task.async(fn ->
          ExLLM.Infrastructure.CircuitBreaker.call_with_bulkhead("full_queue", fn ->
            send(parent, {:started, 1})
      ...
      metrics = ExLLM.Infrastructure.CircuitBreaker.Bulkhead.get_metrics("full_queue")
      assert metrics.active_count == 1
      assert metrics.queued_count == 1
    ...
    test "times out queued requests" do
      ExLLM.Infrastructure.CircuitBreaker.Bulkhead.configure("timeout_test",
        max_concurrent: 1,
        max_queued: 1,
        queue_timeout: 50
      )
      ...
      task1 =
        Task.async(fn ->
          ExLLM.Infrastructure.CircuitBreaker.call_with_bulkhead("timeout_test", fn ->
      ...
      metrics = ExLLM.Infrastructure.CircuitBreaker.Bulkhead.get_metrics("timeout_test")
      assert metrics.active_count == 1
  ...
  describe "bulkhead metrics" do
    test "tracks active and queued requests" do
      ExLLM.Infrastructure.CircuitBreaker.Bulkhead.configure("metrics_test",
        max_concurrent: 2,
        max_queued: 5
      )
      ...
      tasks =
        for i <- 1..3 do
          Task.async(fn ->
            ExLLM.Infrastructure.CircuitBreaker.call_with_bulkhead("metrics_test", fn ->
      ...
      metrics = ExLLM.Infrastructure.CircuitBreaker.Bulkhead.get_metrics("metrics_test")
      ...
      final_metrics = ExLLM.Infrastructure.CircuitBreaker.Bulkhead.get_metrics("metrics_test")
      assert final_metrics.active_count == 0
      assert final_metrics.queued_count == 0
    end

    test "tracks rejection metrics" do
      ExLLM.Infrastructure.CircuitBreaker.Bulkhead.configure("reject_metrics",
        max_concurrent: 1,
        max_queued: 0
      )
      ...
      _task =
        Task.async(fn ->
          ExLLM.Infrastructure.CircuitBreaker.call_with_bulkhead("reject_metrics", fn ->
            Process.sleep(50)
            :ok
          end)
        end)
      ...
      ExLLM.Infrastructure.CircuitBreaker.call_with_bulkhead("reject_metrics", fn -> :ok end)

      metrics = ExLLM.Infrastructure.CircuitBreaker.Bulkhead.get_metrics("reject_metrics")
      assert metrics.total_rejected > 0
    end
  end
  ...
  describe "integration with circuit breaker" do
    test "bulkhead works with circuit breaker states" do
      ExLLM.Infrastructure.CircuitBreaker.Bulkhead.configure("integrated",
        max_concurrent: 2
      )

      # Circuit breaker + bulkhead
      result =
        ExLLM.Infrastructure.CircuitBreaker.call_with_bulkhead("integrated", fn ->
          {:ok, "success"}
        end)
  ...
  describe "provider-specific bulkheads" do
    test "configures different limits per provider" do
      ...
      Enum.each(providers, fn {provider, config} ->
        ...
        ExLLM.Infrastructure.CircuitBreaker.Bulkhead.configure(circuit, config)

        actual = ExLLM.Infrastructure.CircuitBreaker.Bulkhead.get_config(circuit)
