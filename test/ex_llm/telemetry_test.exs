defmodule ExLLM.TelemetryTest do
  ...
  describe "telemetry events" do
    ...
    test "span/3 emits start, stop, and measurements" do
      result =
        ExLLM.Telemetry.span([:test, :operation], %{test: true}, fn ->
          Process.sleep(10)
          {:ok, "result"}
        end)
    ...
    test "span/3 emits exception event on error" do
      assert_raise RuntimeError, "test error", fn ->
        ExLLM.Telemetry.span([:test, :failing], %{}, fn ->
          raise "test error"
        end)
      end
    ...
    test "cache event helpers" do
      ExLLM.Telemetry.emit_cache_hit("test-key")
      assert_receive {:telemetry_event, [:ex_llm, :cache, :hit], %{}, %{key: "test-key"}}

      ExLLM.Telemetry.emit_cache_miss("test-key")
      assert_receive {:telemetry_event, [:ex_llm, :cache, :miss], %{}, %{key: "test-key"}}

      ExLLM.Telemetry.emit_cache_put("test-key", 1024)
    ...
    test "cost event helpers" do
      ExLLM.Telemetry.emit_cost_calculated(:openai, "gpt-4", 150)

      assert_receive {:telemetry_event, [:ex_llm, :cost, :calculated], %{cost: 150},
                      %{provider: :openai, model: "gpt-4"}}

      ExLLM.Telemetry.emit_cost_threshold_exceeded(500, 300)
    ...
    test "stream event helpers" do
      ExLLM.Telemetry.emit_stream_start(:anthropic, "claude-3")

      assert_receive {:telemetry_event, [:ex_llm, :stream, :start], %{system_time: _},
                      %{provider: :anthropic, model: "claude-3"}}

      ExLLM.Telemetry.emit_stream_chunk(:anthropic, "claude-3", 128)

      assert_receive {:telemetry_event, [:ex_llm, :stream, :chunk], %{size: 128},
                      %{provider: :anthropic, model: "claude-3"}}

      ExLLM.Telemetry.emit_stream_complete(:anthropic, "claude-3", 10, 5000)
    ...
    test "metadata enrichment" do
      _result =
        ExLLM.Telemetry.span([:test, :enriched], %{base: "value"}, fn ->
    ...
    test "cost metadata enrichment" do
      _result =
        ExLLM.Telemetry.span([:test, :with_cost], %{}, fn ->
  ...
  describe "default logger" do
    test "attach and detach default logger" do
      ...
      # Should log events
      ExLLM.Telemetry.emit_cache_hit("test")

      # Should detach without error
      ExLLM.Telemetry.detach_default_logger()
    end
  end

  describe "all defined events" do
    test "returns comprehensive event list" do
      events = ExLLM.Telemetry.events()
